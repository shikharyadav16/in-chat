...
class RoomValidator {
    static validateRoomData(room) {
        if (!room.name || typeof room.name !== 'string' || room.name.trim() === '' || room.name.length < 3 || room.name.length > 50) {
            throw new RoomValidationError('Room name is required and must be a string between 3 and 50 characters.');
        }

        if (room.description && (typeof room.description !== 'string' || room.description.length > 200)) {
            throw new RoomValidationError('Room description must be a string with a maximum length of 200 characters.');
        }
